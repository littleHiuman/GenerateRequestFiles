...
const calculateObj = {
      ...
      for (const key in element) {
        if (Object.hasOwnProperty.call(element, key)) {
          ...
          if (secKeys.length > 1) {
            ...
            calculateObj.temFns(url, urlsObjInfo, key, url, value, definitions)
          }
        }
      }
    }
  ...
  temFns(fnsName, urlsObjInfo, method, url, obj, definitions) {
    ...
    const element = calculateObj.calcRequestParams(obj, method, url)
    if (Object.keys(element.param).length) {
      calculateObj.calcDefinitions(definitions, element)
    }
  ...
  calcRequestParams: function (obj, method, url) {
    let param = {}
    if (obj.parameters) {
      param = calculateObj.calcParameters(obj.parameters, method)
    }
    const info = {
      desc: obj.summary || '',
      param,
    ...
    for (const p in param) {
      if (Object.hasOwnProperty.call(param, p)) {
        ...
        if (obj.definitions) {
          ...
          let temObj = calculateObj.temFns2(info, definitions, p, obj.required)

          calculateObj.concatParam(element, p, temObj)
        }
        ...
        if (Object.hasOwnProperty.call(properties, property)) {
          ...
          if ($ref) {
            ...
            if (refVal.properties) {
              ...
              let afterRes = calculateObj.handleNestingParams(
                property,
                res,
...
`
  ...
  calcNameStr: function (urlKey, url, allNames) {
    ...
    if (urlKey !== url) {
      ...
      let { calcName } = calculateObj.calcName(url)
      res = `${prefix}${calcName}`
    } else {
      let { calcName, calcNameResult } = calculateObj.calcName(url, 1)
